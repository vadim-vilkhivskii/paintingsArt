...
var accordion = function accordion(triggers) {
  var btns = document.querySelectorAll(triggers);
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var statusMessage = document.createElement('div'); //создаем новый блок с сообщением статуса
      ...
      var statusImg = document.createElement('img'); //создаем элемент с оповещением
      ...
      var formData = new FormData(item); //собираем данные с формы для дальнейшей отправки
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      ...
      document.querySelectorAll('[data-modal]').forEach(function (item) {
        //проверка на наличие уже открытых окон
...
var showMoreStyles = function showMoreStyles(triger, wrapper) {
  var btn = document.querySelector(triger); // cards.forEach(card => {
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1,
      //текущий слайд
  paused = false;
  var items = document.querySelectorAll(slides); //список слайдов
  ...
  try {
    var prevBtn = document.querySelector(prev),
        //предыдущий слайд
    nextBtn = document.querySelector(next); //следующий
